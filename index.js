...
const drawBoard = () => {
  boxes.forEach((box, index) => {
    ...
    box.addEventListener("keydown", boxKeyPressed);

  });
};

function boxKeyPressed(e){
  let id = e.target.id[1];
  let key = e.key;
  
  if(key === 'ArrowRight' && Number(id) < 8){
      id = (Number(id) + 1).toString();
  }else if(key === 'ArrowLeft' && Number(id) > 0){
      id = (Number(id) - 1).toString();
  }else if(key === 'ArrowUp' && Number(id) > 2){
      id = (Number(id) - 3).toString();
  }else if(key === 'ArrowDown' && Number(id) < 7){
      id = (Number(id) + 3).toString();
  }else if(key === 'Enter'){
    boxClicked(e);
  }
  currentCell = document.querySelector(`#c${id}`);
  if(currentCell){
      currentCell.focus();
  }
}
